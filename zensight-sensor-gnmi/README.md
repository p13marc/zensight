...
gNMI (gRPC Network Management Interface) sensor for the ZenSight observability platform. Subscribes to streaming telemetry from network devices and publishes it to Zenoh.
...
zensight-sensor-gnmi --config configs/gnmi.json5

# Run with custom config path
zensight-sensor-gnmi --config /etc/zensight/gnmi.json5
```
...
cargo test -p zensight-sensor-gnmi

# Run path tests only
cargo test -p zensight-sensor-gnmi path

# Run with verbose output
cargo test -p zensight-sensor-gnmi -- --nocapture
```
