...
//!
//! Exit codes separate the two ways this can come back empty: **2** when the
//! session could not be opened at all (wrong endpoint, nothing listening) and
//! **1** when the endpoint answered but no sample ever arrived (wrong selector,
//! or a genuinely silent fleet). A zero-length capture is never success.
//!
//! ```bash
//! # env
...
async fn main() {
    ...
    config
        ...
        .unwrap();
    // Not `.expect()`: "cannot reach the bus" and "reached the bus, nothing
    // published" are different failures with different fixes, and a panic
    // renders the first as a stack trace in a log the operator has to go
    // find. Exit 2 says the endpoint is wrong; exit 1 (below) says the
    // endpoint was right and the fleet was silent.
    let session = match zenoh::open(config).await {
        Ok(s) => s,
        Err(e) => {
            eprintln!(
                "cannot open a session to {connect}: {e}\n\
                 Nothing is listening there. This is not a quiet fleet — it is an \
