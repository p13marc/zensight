...
System monitoring sensor for the ZenSight observability platform. Collects local host metrics and publishes them to Zenoh.
...
zensight-sensor-sysinfo --config configs/sysinfo.json5

# Run with custom config path
zensight-sensor-sysinfo --config /etc/zensight/sysinfo.json5
```
...
cargo test -p zensight-sensor-sysinfo

# Run config tests only
cargo test -p zensight-sensor-sysinfo config

# Run with verbose output
cargo test -p zensight-sensor-sysinfo -- --nocapture
```
...
The sensor uses the `sysinfo` crate which supports:
...
```ini
# /etc/systemd/system/zensight-sensor-sysinfo.service
[Unit]
Description=ZenSight Sysinfo Sensor
After=network.target

[Service]
Type=simple
ExecStart=/usr/local/bin/zensight-sensor-sysinfo --config /etc/zensight/sysinfo.json5
Restart=always
RestartSec=5
...
```bash
sudo systemctl enable zensight-sensor-sysinfo
sudo systemctl start zensight-sensor-sysinfo
```
...
WORKDIR /app
COPY . .
RUN cargo build -p zensight-sensor-sysinfo --release

FROM debian:bookworm-slim
COPY --from=builder /app/target/release/zensight-sensor-sysinfo /usr/local/bin/
COPY configs/sysinfo.json5 /etc/zensight/
CMD ["zensight-sensor-sysinfo", "--config", "/etc/zensight/sysinfo.json5"]
```
