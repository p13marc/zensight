...
async fn main() -> Result<()> {
    ...
    // Installed through `adopt` on the collector's registry (#1155), which
    // `Collector::new` builds — the runner's publisher carries none of
    // netlink's 106 metric families.
    //
