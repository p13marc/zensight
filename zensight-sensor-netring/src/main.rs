...
async fn main() -> Result<()> {
    ...
    // On-demand debug-report (`@rpc/netring/artifact/*`): bundle redacted config + health +
    ...
    let producer = "netring".to_string();

    tracing::info!(
        "Netring sensor running (producer: {}, source: {}, capture: {})",
        producer,
        source,
    ...
    // the off-hook socket-table refresh and serve `@rpc/netring/bandwidth` here.
    ...
    // identity evidence on `state/netring/evidence/**` for the correlator.
    ...
    // evidence on `state/netring/evidence/names/*` for the correlator.
