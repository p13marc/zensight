...
| `hot_secs` | `600` | Seconds of history held **in memory, per series** — bounded in time, and by an element ceiling of the same number, whichever binds first. Ten minutes is enough for a live chart to be served from the ring; it is also the term that grows with the fleet, and the governor halves it first under pressure — both bounds, so the log line stays true. (#1065 — until then this was a raw *sample count*, so a 10 s-interval series held a hundred minutes and ten times the intended memory, and "ten minutes" was true only for a 1 Hz series.) |
...
| `batch_size` | `4096` | Samples buffered, across every series, before a flush is triggered early — the lever for an ingest burst, which is the pressure `budget_rss_mb` exists for. (#1066 — until then it was documented, validated `> 0` and read by nothing: flushes were interval-only and the buffer grew for the whole window. The depth it compares against is a maintained counter since #1211; it was a walk of the series map, asked on every point, inside the lock the query handlers take.) |
