...
| `batch_size` | `4096` | Samples buffered, across every series, before a flush is triggered early — the lever for an ingest burst, which is the pressure `budget_rss_mb` exists for. (#1066 — until then it was documented, validated `> 0` and read by nothing: flushes were interval-only and the buffer grew for the whole window.) |
