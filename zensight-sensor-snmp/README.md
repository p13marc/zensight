...
SNMP sensor for the ZenSight observability platform. Polls SNMP devices and publishes telemetry to Zenoh.
...
zensight-sensor-snmp --config configs/snmp.json5

# Run with custom config path
zensight-sensor-snmp --config /etc/zensight/snmp.json5
```
...
The sensor can load MIB files to automatically resolve OID names:
...
When `trap_listener` is enabled, the sensor:
...
cargo test -p zensight-sensor-snmp

# Run with verbose output
cargo test -p zensight-sensor-snmp -- --nocapture
```
