...
use std::net::IpAddr;
use zensight_common::hostspec::ListenProto;
...
pub struct ListenEntry {
    pub addr: IpAddr,
    pub port: u16,
    /// Which `/proc/net` table it came from (#1138).
    pub proto: ListenProto,
}

/// The `st` column of a LISTENing TCP socket.
const TCP_LISTEN: &str = "0A";
/// The `st` column of a bound UDP socket. UDP has no listen state — `07` is
/// `TCP_CLOSE`, which is what an unconnected datagram socket reports, and it
/// is what `ss -ulnp` counts as listening.
...
pub fn parse_proc_net_tcp(text: &str, v6: bool) -> Vec<ListenEntry> {
    parse_proc_net(text, v6, ListenProto::Tcp)
}

/// Parse `/proc/net/udp` (or `udp6` with `v6 = true`), bound sockets only
/// (#1138).
///
/// Same table layout as TCP — `/proc/net/udp` is generated by the same
/// `seq_file` shape — with a different state to select for.
pub fn parse_proc_net_udp(text: &str, v6: bool) -> Vec<ListenEntry> {
    parse_proc_net(text, v6, ListenProto::Udp)
}

fn parse_proc_net(text: &str, v6: bool, proto: ListenProto) -> Vec<ListenEntry> {
    let want_state = match proto {
        ListenProto::Udp => UDP_BOUND,
        _ => TCP_LISTEN,
    };
    let mut out = Vec::new();
    for line in text.lines().skip(1) {
        ...
        if cols.len() < 4 || cols[3] != want_state {
            continue;
        }
        ...
        out.push(ListenEntry { addr, port, proto });
    }
    out
}
...
/// Every TCP **and UDP** listener visible in THIS network namespace (#1138) —
/// `/proc/net/{tcp,tcp6,udp,udp6}`. A containerized service's listener lives
/// in its own namespace and is invisible here — documented, not worked
/// around.
///
/// Unreadable only when **no** table could be read. A kernel built without
/// IPv6 has no `tcp6`/`udp6`, and that is an absent table rather than a
/// failure to observe.
pub fn read_listeners() -> Observation<Vec<ListenEntry>> {
    let tables = [
        ("/proc/net/tcp", false, ListenProto::Tcp),
        ("/proc/net/tcp6", true, ListenProto::Tcp),
        ("/proc/net/udp", false, ListenProto::Udp),
        ("/proc/net/udp6", true, ListenProto::Udp),
    ];
    let mut out = Vec::new();
    let mut read_any = false;
    let mut first_error = None;
    for (path, v6, proto) in tables {
        match std::fs::read_to_string(path) {
            Ok(t) => {
                read_any = true;
                out.extend(parse_proc_net(&t, v6, proto));
            }
            Err(e) => {
    ...
    if read_any {
        ...
        Observation::Unreadable(first_error.unwrap_or_else(|| "no /proc/net tables".into()))
    }
}
