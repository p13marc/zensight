...
| `listening` | `port`, `proto?`, `addr?`, `dual_stack?`, `forbid` | TCP **and UDP** listeners in **this network namespace** (`/proc/net/{tcp,tcp6,udp,udp6}`; v4-mapped v6 normalizes to v4). `proto` is `tcp` (default) · `udp` · `both`. `addr` is an exact bound-address match; `0.0.0.0` and `::` are **distinct** wildcards — forbid both, or set `dual_stack: true` to mean "either wildcard" in one expectation. `forbid: true` fires when a matching listener exists. |
...
### UDP is not an afterthought (#1138)

`listening` read `/proc/net/tcp{,6}` only, and `ListeningExpectation` had no
protocol field, so there was no way to say otherwise. Two consequences, and the
second is the one that matters:

- `{port: 53}` for a resolver, or `{port: 514}` for a syslog receiver, was a
  permanent **false "nothing is listening"**;
- `{port: 53, forbid: true}` — which is how an operator writes *"this host must
  not expose an open resolver"* — was a **false all-clear**, from the sensor
  whose entire purpose is machine-checked assertions.

`proto` defaults to `tcp`, so every expectation written before this means
exactly what it meant and matches exactly what it matched. A `tcp` expectation
is **not** satisfied by a UDP listener on the same port; `both` is satisfied by
either.

UDP has no listen state. The parser selects `st == 07` (`TCP_CLOSE`), which is
what an unconnected datagram socket reports and what `ss -ulnp` counts as
listening — the same table layout as TCP, since `/proc/net/udp` is generated by
the same `seq_file` shape.

`read_listeners` is `Unreadable` only when **no** table could be read: a kernel
built without IPv6 has no `tcp6`/`udp6`, and that is an absent table rather than
