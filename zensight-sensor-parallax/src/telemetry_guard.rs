...
/// `zensight-keyspace/registry/parallax.toml` fails the existing tests.
pub(crate) fn checked_point(
    ...
    debug_assert!(
        zensight_keyspace::registry::is_registered_telemetry("parallax", &metric),
        "unregistered parallax telemetry subject {metric:?} — add it to \
         zensight-keyspace/registry/parallax.toml (RFC 08 §5, issue #468)"
    );
    TelemetryPoint::new(source, Protocol::Parallax, metric, value)
}
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "unregistered parallax telemetry subject")]
    ...
    fn a_registered_metric_constructs() {
        let p = checked_point("h", "streams/advertised", TelemetryValue::Gauge(1.0));
        assert_eq!(p.metric, "streams/advertised");
    }
}
