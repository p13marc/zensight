...
Modbus sensor for the ZenSight observability platform. Polls Modbus devices and publishes register values to Zenoh.
...
- **Multi-device** - Poll multiple devices from single sensor instance
...
zensight-sensor-modbus --config configs/modbus.json5

# Run with custom config path
zensight-sensor-modbus --config /etc/zensight/modbus.json5
```
...
cargo test -p zensight-sensor-modbus

# Run decoder tests only
cargo test -p zensight-sensor-modbus decode

# Run with verbose output
cargo test -p zensight-sensor-modbus -- --nocapture
```
