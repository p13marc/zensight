//! Cross-sensor correlation registry.
//!
//! This module provides device identity correlation across protocol sensors.
...
//!
//! ```ignore
//! use zensight_sensor_core::{CorrelationRegistry, DeviceIdentity};
//!
//! let registry = CorrelationRegistry::new(publisher);
//!
//! // Register a device seen by the SNMP sensor
...
pub struct DeviceIdentity {
    ...
    /// Sensor that registered this identity.
    pub sensor: String,
    /// Source identifier used by the sensor.
    pub source_id: String,
}

/// Correlation entry aggregating identities across sensors.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl CorrelationEntry {
    ...
    fn from_identity(identity: &DeviceIdentity) -> Self {
        ...
        sources.insert(identity.sensor.clone(), identity.source_id.clone());

        Self {
            ip: identity.ip,
            hostnames: identity.hostnames.clone(),
            sensors: vec![identity.sensor.clone()],
            sources,
    ...
    fn merge(&mut self, identity: &DeviceIdentity) {
        ...
        // Add sensor if not present
        if !self.sensors.contains(&identity.sensor) {
            self.sensors.push(identity.sensor.clone());
        }

        // Update source mapping
        self.sources
            .insert(identity.sensor.clone(), identity.source_id.clone());

        self.last_updated = chrono::Utc::now().timestamp_millis();
    }
}

/// Registry for cross-sensor device correlation.
///
...
impl CorrelationRegistry {
    ...
    ///
    /// Convenience method for sensors that don't have full identity info.
...
impl SensorInfo {
    ...
    /// Publish sensor info to Zenoh.
    pub async fn publish(&self, publisher: &Publisher) -> Result<()> {
        let key = format!("zensight/_meta/sensors/{}", self.name);
...
mod tests {
    ...
    async fn test_register_device() {
        ...
        assert_eq!(entry.sensors, vec!["snmp"]);
    ...
    async fn test_merge_identities() {
        ...
        assert!(entry.sensors.contains(&"snmp".to_string()));
        assert!(entry.sensors.contains(&"syslog".to_string()));
    ...
    #[test]
    fn test_sensor_info() {
        let info = SensorInfo::new("snmp", "0.1.0", "zensight/snmp", "snmp").with_device_count(10);
