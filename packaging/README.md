...
Exposure is `systemd-analyze security --offline=true` on the `.service`
(#1204) — generated with the rest of the row, so a unit that loses hardening
shows up here as a stale-table diff rather than in a re-measurement months
later. Capabilities are the `.service`'s `AmbientCapabilities` without the
`CAP_` prefix; the quadlet's `AddCapability` must match. `budget_rss_mb` is
