...
it subscribes to `zensight/**` over Zenoh, auto-discovers every sensor, groups
each host's per-protocol facets under one card, and rolls firing alerts up into
unified incidents. Telemetry persists to a bounded local store so history
survives restart.
