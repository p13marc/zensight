...
/// `zenkey/registry/ (zenkey repo)logs.toml` fails the existing tests.
pub(crate) fn checked_point(
    ...
    debug_assert!(
        zenkey::registry::is_registered_telemetry("logs", &metric),
        "unregistered logs telemetry subject {metric:?} — add it to \
         zenkey/registry/ (zenkey repo)logs.toml (RFC 08 §5, issue #468)"
    );
