//! NetFlow/IPFIX sensor configuration.
...
/// Complete NetFlow sensor configuration.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct NetFlowSensorConfig {
    ...
    /// `@rpc/netflow/thresholds/set`; `state/netflow/applied/thresholds`
...
mod shipped_config {
    ...
    fn shipped_config_parses() {
        let path = concat!(env!("CARGO_MANIFEST_DIR"), "/../configs/netflow.json5");
        let _config = crate::config::NetFlowSensorConfig::load_from_file(path)
            .expect("configs/netflow.json5 must load");
    }
}
