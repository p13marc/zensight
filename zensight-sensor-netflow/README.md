...
NetFlow/IPFIX sensor for the ZenSight observability platform. Receives network flow data and publishes it to Zenoh.
...
zensight-sensor-netflow --config configs/netflow.json5

# Run with custom config path
zensight-sensor-netflow --config /etc/zensight/netflow.json5
```
...
cargo test -p zensight-sensor-netflow

# Run parser tests only
cargo test -p zensight-sensor-netflow v5
cargo test -p zensight-sensor-netflow v9

# Run with verbose output
cargo test -p zensight-sensor-netflow -- --nocapture
```
